...
H_matrix::H_matrix(const tensor_lattice &K, const tensor_matrix &U_K, vec3 k_vec)
{
    size = K.size;
...
// Update the H_block for new q value (just the diagonal)
void H_matrix::update_H_block(const tensor_lattice &K, const double U_K_0, vec3 k_vec)
{
    int N = K.size;
...
void H_matrix::calculate_eigenvalues()
{
